	...
	public class SearchBarAnimationBehavior : Behavior<SfButton>
	{
			...
			if (button != null)
			{
				if (this.AnimationType == AnimationType.expand)
				{
					...
					if (children != null)
					{
						...
						searchLayout.Width,
						Easing.Linear);
						expandAnimation.Commit(searchLayoutChildren, "Expand", 16, 250, Easing.Linear, (p, q) => this.SearchExpandAnimationCompleted(children.Cast<View>().ToList()));
					}
				}
				else if (this.AnimationType == AnimationType.shrink)
				{
					...
					if (children != null)
					{
						var searchEntry = (children[1] as Border).Content;
		...
		private void SearchExpandAnimationCompleted(IList<View> children)
		{
			if (children != null)
			{
				...
				var searchEntry = (searchLayout.Children[1] as Border).Content;
